...
class CancelSubscriptionJob < ApplicationJob
  ...
  def perform(subscription_id)
    ...
    Rails.logger.info "Starting cancellation process for subscription #{subscription.name} (ID: #{subscription.id})"
    ...
    begin
      ...
      # Revoke WireGuard clients if they exist
      revoke_wireguard_clients(subscription)

      Rails.logger.info "Successfully canceled subscription #{subscription.name}"
    rescue StandardError => e
      Rails.logger.error "Error canceling subscription #{subscription.name}: #{e.message}"
  ...
  def revoke_wireguard_clients(subscription)
    return unless subscription.server
  ...
  def revoke_client_with_retries(server, wireguard_client, private_key_path, retries = 0)
    ...
    begin
      ...
      if retries < 3 # Maximum of 3 retries
        sleep(2 ** retries) # Exponential backoff
        Rails.logger.info "Retrying (#{retries + 1}/3) to revoke client #{wireguard_client.name}"
        retry
      else
        Rails.logger.error "Failed to revoke client #{wireguard_client.name} after 3 attempts: #{e.message}"
      end
    end
  end
