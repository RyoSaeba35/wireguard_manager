...
class ClashApiMonitorJob < ApplicationJob
  ...
  CLASH_API_PORT = 9090
  WIREGUARD_HANDSHAKE_TIMEOUT = 5.minutes
  ...
  def perform
    ...
    devices_with_recent_heartbeats = Device
      ...
      .where('subscriptions.expires_at IS NULL OR subscriptions.expires_at > ?', Time.current)
      .pluck('devices.id')
  ...
  def force_release_abandoned_configs
    abandoned_configs = VpnConfigSet
      .where(status: 'in_use')
      .joins('INNER JOIN devices ON devices.id = vpn_config_sets.device_id')
      .where(
    ...
    connections_by_ip.each do |source_ip, conns|
      ...
      if source_ip.start_with?('10.', '172.16.', '192.168.')
        ...
        if config_set
          ...
          Rails.logger.info "ℹ️ Internal IP #{source_ip} not matched — skipping"
        end
        next
      end
      ...
      if config_set
        ...
        # Strategy 2: Match by external IP using raw join to avoid scope conflict
        device = Device.where(active: true)
          .where('last_connection_ip::text = ?', source_ip)
          .joins('INNER JOIN vpn_config_sets ON vpn_config_sets.device_id = devices.id')
        ...
        if device
          ...
          # Strategy 3: Recent heartbeat using raw join to avoid scope conflict
          device = Device.where(active: true)
            .where('devices.last_seen_at > ?', 2.minutes.ago)
            .joins('INNER JOIN vpn_config_sets ON vpn_config_sets.device_id = devices.id')
            .where(vpn_config_sets: { server_id: server.id, status: 'in_use' })
            .first
      ...
      if device.nil?
        # Only kill if truly no active devices on this server
        active_devices_on_server = Device.where(active: true)
          .joins('INNER JOIN vpn_config_sets ON vpn_config_sets.device_id = devices.id')
          .where(vpn_config_sets: { server_id: server.id, status: 'in_use' })
          .count
        ...
        if active_devices_on_server == 0
          ...
          Rails.logger.warn "⚠️ UNKNOWN IP: #{source_ip} (#{conns.size} connections) — #{active_devices_on_server} active device(s) on server, skipping kill"
        end
        next
      end
      ...
      unless device.subscription&.active?
        ...
        conns.each { |conn| kill_connection_via_ssh(server, conn["id"]) }
  ...
  # Secret passed via stdin — never appears in process list or shell history
  def get_active_connections_via_ssh(server)
    ...
    begin
      ...
      Net::SSH.start(
        ...
        command = "echo 'Authorization: Bearer #{server.clash_api_secret}' | curl -s -H @- http://127.0.0.1:#{CLASH_API_PORT}/connections"
  ...
  def kill_connection_via_ssh(server, connection_id)
    ...
    begin
      ...
      Net::SSH.start(
        ...
        command = "echo 'Authorization: Bearer #{server.clash_api_secret}' | curl -s -X DELETE -H @- http://127.0.0.1:#{CLASH_API_PORT}/connections/#{connection_id}"
        ssh.exec!(command)
  ...
  def release_configs_for_devices(device_ids)
    ...
    VpnConfigSet.where(device_id: device_ids, status: 'in_use').find_each do |config_set|
      ...
      if server.singbox_active? && server.clash_api_secret.present?
        begin
          killed_count = kill_singbox_connections_for_vpn_ip(server, config_set.ip_address)
          Rails.logger.info "🔪 Killed #{killed_count} sing-box connections for device #{device.id} (VPN IP: #{config_set.ip_address})"
        rescue => e
  ...
  def kill_wireguard_peer(server, vpn_ip)
    ...
    begin
      ...
      Rails.logger.error "Failed to kill WireGuard peer #{vpn_ip} on #{server.name}: #{e.message}"
    ensure
      if key_file
        key_file.close unless key_file.closed?
        key_file.unlink
